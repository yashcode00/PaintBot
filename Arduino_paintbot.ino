#include <Servo.h> //servo motor for controlling the paint trigger
#include <Stepper.h> //stepper motors for controlling the motion of the paintbot
#include <LiquidCrystal.h>
LiquidCrystal lcd(A0, A1, A2, A3, A4, A5);
#define LINE_BUFFER_LENGTH 512

// Servo position for  off and  on 
...
lcd.setCursor(5,0);
lcd.print("WELCOME");
paintServo.attach(paintServoPin);
...
Serial.println("Paintbot alive and kicking!");
    ...
    while ( Serial.available()>0 ) {
      ...
      if (( c == '\n') || (c == '\r') ) {             // End of line reached
        ...
        if ( (lineIsComment) || (lineSemiColon) ) {   // Throw away all comment characters
          ...
          else if ( c == '/' ) {                    // Block delete not s offported. Ignore character.
          } 
          ...
          else if ( c >= 'a' && c <= 'z' ) {        //  offcase lowercase
...
lcd.setCursor(8,0);lcd.print("Y=");lcd.print(y1);lcd.print("  ");
  ...
  delay(LineDelay);
  //   offdate the positions
  Xpos = x1;
  Ypos = y1;
}

//  stops spraying paint
void paintoff() { 
  paintServo.write(paintZoff); 
  delay(LineDelay); 
  Zpos=Zmax; 
